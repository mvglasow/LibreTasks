...
/**
 * This class encapsulates a WifiDisconnected event. It wraps the intent that triggered this event
 * and provides access to any attribute data associated with it.
 */
public class WifiDisconnectedEvent extends Event {
	/** Event name (to match record in database) */
	public static final String APPLICATION_NAME = "WiFi";
	public static final String EVENT_NAME = "WiFi Disconnected";
	public static final String ACTION_NAME = "WIFI_DISCONNECTED";
	...
	public WifiDisconnectedEvent(Intent intent) {
		super(APPLICATION_NAME, EVENT_NAME, intent);
		ssid = intent.getStringExtra(WifiDisconnectedEvent.ATTRIBUTE_WIFI_SSID);
		bssid = intent.getStringExtra(WifiDisconnectedEvent.ATTRIBUTE_WIFI_BSSID);
	}

	@Override
	public String getAttribute(String attributeName) throws IllegalArgumentException {
		if (attributeName.equals(WifiDisconnectedEvent.ATTRIBUTE_WIFI)) {
			try {
		...
		} else if (attributeName.equals(WifiDisconnectedEvent.ATTRIBUTE_WIFI_SSID)) {
			return ssid;
		} else {
